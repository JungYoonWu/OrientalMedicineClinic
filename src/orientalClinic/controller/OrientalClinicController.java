...
import orientalClinic.model.user.Admin;
...
public class OrientalClinicController {
	...
	Cart mCart;
	ReservationCalendar mRezCalender;
	Customer mCustomer;
	Admin mAdmin;
	...
	public OrientalClinicController(HerbologyStorage herbologyStorage, Cart cart, ReservationCalendar reservations,ConsoleView view) {
		this.view = view;
		this.mHerbologyStorage = herbologyStorage;
		this.mRezCalender = reservations;
		this.mCart = cart;
	...
	//예약목록 1번
	private void viewCanlanderRezInfo() {
		view.displayRez(mRezCalender.getReservations());
	}
	
	//전체 예약 목록 2번
	private void viewTotalCalenderInfo() {
		view.displayRez(mRezCalender.getReservations());
	}
	
	//진료 예약 3번
	private void reservationDiagnosis(){
		String date = view.selectRezDate();
		String time = view.selectRezTime();
		Reservation reservation = new Reservation(date, time, mCustomer.getName(), mCustomer.getPhone());
		try {
			mRezCalender.addReservation(reservation);
			view.showMessage("예약이 완료 되었습니다.");
		}catch(IOException e) {
			view.showMessage("예약이 실패했습니다.");
		}
	}
	...
	private void cancelReservation() {
		view.displayRez(mRezCalender.getReservations());
		try {
			if(!mRezCalender.isEmpty()) {
				String date = view.selectRezDate();
				String time = view.selectRezTime();
				if(view.askConfirm(">>해당 예약을 삭제하려면 yes를 입력하세요 : ", "yes")) {
					mRezCalender.removeReservation(date, time, mCustomer.getName(), mCustomer.getPhone());
					view.showMessage("진료예약을 취소했습니다.");
				}
			}
			
		}catch(IOException e) {
			view.showMessage("예약 취소가 실패했습니다.");
		}
	}
	...
	private void addHerbToCart() {
		...
		view.showMessage(">>장바구니에 한약을 추가하였습니다.");	
	}
	...
	private void updateHerbInCart() {
		...
		if (!mCart.isEmpty()) {
			...
			mCart.updateQuantity(herbId, quantity);
		}
	}
	...
	private void order() {
		getDeliveryInfo();
		viewOrderInfo();
		view.showMessage("전체 주문 금액은 " + mCart.getTotalPrice() + "입니다.");
		if(view.askConfirm("진짜 주문하려면 yes를 입력하세요 : ", "yes"));{
			view.showMessage("주문 감사합니다.");
			mCart.resetCart();
		}
	}
